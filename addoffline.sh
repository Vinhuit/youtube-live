...
do
  ...
  if [ -z "$2" ]; then
    	  STATUS=$(curl -k -s -o /dev/null -w '%{http_code}' -i -H "Accept: application/json" -H "Content-Type:application/json" -X PUT --data "[{\"device\":\"$DEVICE\"}]" "http://xjsonserver01.herokuapp.com/offline/$1")
	    else
	    	  times=$(grep -oP 'time=...........' log.txt | tail -1| cut -f2 -d '=' )
		  	  echo $times
			  	  STATUS=$(curl -k -s -o /dev/null -w '%{http_code}' -i -H "Accept: application/json" -H "Content-Type:application/json" -X PUT --data "[{\"key\":\"$1\",\"link\":\"$2\",\"device\":\"$DEVICE\",\"times\":\"$times\"}]" "http://xjsonserver01.herokuapp.com//online/$4")
